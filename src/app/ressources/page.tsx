...
import { Search, BookOpen, FileText, Users, Calendar, Tag, ArrowRight } from 'lucide-react';
  ...
  try {
                ...
                <article key={post.id} className="card overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xs font-medium text-or bg-amber-50 px-2 py-1 rounded">
                        Article
                      </span>
                      <span className="text-xs text-gray-500">5 min</span>
                    </div>
                    <h3 className="text-lg font-playfair font-semibold text-anthracite mb-3 line-clamp-2">
                      {post.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex flex-wrap gap-1">
                        {post.post_tags?.slice(0, 2).map((postTag: any) => (
                          <span key={postTag.tags.id} className="text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded">
                            {postTag.tags.name}
                          </span>
                        ))}
                      </div>
                      <Link
                        ...
                        className="text-or hover:text-yellow-600 text-sm font-medium flex items-center"
                      >
                        Lire
              ...
              <div className="lg:col-span-2">
                ...
                <div className="space-y-6">
                  {recentPosts && recentPosts.length > 0 ? recentPosts.map((post) => (
                    <article key={post.id} className="bg-white p-6 rounded-sm shadow-sm hover:shadow-md transition-shadow">
    ...
    return (
      <div className="section-padding bg-white">
        <div className="container-custom text-center">
          <h1 className="text-4xl font-playfair font-bold text-noir mb-6">
            Ressources & Guides
          </h1>
